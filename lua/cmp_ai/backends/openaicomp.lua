...
OpenAIComp = requests:new(nil)
BASE_URL = 'https://api.openai.com/v1/chat/completions'

function OpenAIComp:new(o, params)
  o = o or {}
  setmetatable(o, self)
  self.__index = self
  ...
  self.base_url = params.base_url or BASE_URL
  self.api_key = params.api_key or os.getenv('OPENAI_API_KEY')
...
function OpenAIComp:complete(lines_before, lines_after, cb)
  if not self.api_key then
    vim.schedule(function()
      vim.notify('OPENAI_API_KEY environment variable not set', vim.log.levels.ERROR)
    end)
...
Your answer should be:
  ...
  self:Get(self.base_url, self.headers, data, function(answer)
    local new_data = {}
